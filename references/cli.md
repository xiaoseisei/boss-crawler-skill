...
| 可读投递材料 | `write_application_md.py <run_dir> --all` | `qualified_jobs.json` + 原始 CSV + 招呼语 + `materials/resume_*.json` | `deliver/<公司>-<岗位>/岗位信息+招呼语.md`、`优化建议.md` |
...
│       ├── 岗位信息+招呼语.md                  岗位全字段 + 招呼语 + 匹配分析（materials 后自动写）
│       └── 优化建议.md              给「改简历」的优化建议（跟岗位信息+招呼语.md 一并生成）
...
三态语义 `0 可用 / 1 缺失或非法 / 2 已配置但不可达` 见 [SKILL.md 前置条件：LLM 配置](../SKILL.md#前置条件llm-配置)）。
